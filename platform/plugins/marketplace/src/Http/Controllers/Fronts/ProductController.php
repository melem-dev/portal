...
class ProductController extends BaseController
{
    ...
    )
    {
        ...
        $priceOriginal = str_replace('.', '', $request->input('price_2'));
        $priceOriginal = str_replace(',', '.', $priceOriginal);

        $priceSale = str_replace('.', '', $request->input('sale_price_2'));
        $priceSale = str_replace(',', '.', $priceSale);

        $throw = (30 / 100 * $priceOriginal);
        if (($priceOriginal - $priceSale) < $throw) {
            $throw = number_format($throw, 2, ',', '.');
            return $response
                ->setError()
                ->setMessage("O desconto concedido não pode ser inferior a R$ {$throw} (30%)");
        }
