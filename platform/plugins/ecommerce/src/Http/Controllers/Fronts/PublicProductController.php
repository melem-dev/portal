...
class PublicProductController
{
    ...
    public function getProducts(Request $request, GetProductService $productService, BaseHttpResponse $response)
    {
        ...
        if ($request->has('city_id') && strlen($request->input('city_id')) > 0) {
            $conditions['city_id'] = $request->input('city_id');
        }

        $products = $productService->getProduct($request, null, null, $with, $withCount, $conditions);
    ...
    public function getProduct($slug, Request $request)
    {
        ...
        $product->update(['order' => $product->order + 1]);
