...
class Product extends BaseModel
{
    ...
    protected static function boot()
    {
        ...
        self::creating(function (Product $product) {
            ...
            $product->total_discount = $product->price - $product->sale_price;
            $product->percentage_discount = str_replace('-', '', (($product->price - $product->sale_price) / ($product->price > 0 ?$product->price :  1)) * 100);
        });
     ...
     * @return float
     */
    public function getDiscountAppliedAttribute()
    {
        return (int) preg_replace("/[^0-9]/", "", get_sale_percentage($this->price, $this->front_sale_price, false, ''));
    }
     ...
     * @return float
     */
    public function getOriginalMinusSaleAttribute()
    {
        if ($this->sale_price === 0 || $this->sale_price === null) {
            return 0;
        }
        $total = $this->price - $this->sale_price;

        return (float) ($total < 0) ? 0 : $total;
    }
