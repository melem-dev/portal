...
(function ($) {
  ...
  function sliderImageByResolution() {
    var $slider = $('.single-hero-slider');

    if ($(window).width() >= 1200) {
      if ($slider.data('original-image')) {
        $slider.css({
          'background-image': 'url(' + $slider.data('original-image') + ')'
        });
      }
    } else if ($(window).width() > 768) {
      if ($slider.data('tablet-image')) {
        $slider.css({
          'background-image': 'url(' + $slider.data('tablet-image') + ')'
        });
      }
    } else if ($(window).width() <= 768) {
      if ($slider.data('mobile-image')) {
        $slider.css({
          'background-image': 'url(' + $slider.data('mobile-image') + ')'
        });
      }
    }
  } //Load functions
